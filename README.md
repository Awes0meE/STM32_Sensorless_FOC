...
- `KEY1 / PC4`：电机启停
- `KEY2 / PC5`：切换 OLED 显示
- `KEY3 / PB2`：速度参考在 `1800-3000 rpm` 之间按 `300 rpm` 步进循环

默认启动目标为 `1800 rpm`，当前安全版不会主动给出超过 `3000 rpm` 的速度参考。
...
- 速度范围：`30-50 Hz` 机械频率，即 `1800-3000 rpm`。
- 启动电流：`MOTOR_STARTUP_CURRENT=3.0f`，速度环输出限幅约 `-2A` 到 `4A`。
