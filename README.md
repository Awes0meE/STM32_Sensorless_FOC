...
- `KEY1 / PC4`：电机启停
- `KEY2 / PC5`：切换 OLED 显示
- `EC11 / PB6/PB7`：调节开环目标速度，当前范围 `30-80Hz` 电角频率

当前用于低速产品演示，上电默认设定为 `1200rpm / 80Hz`，旋钮最低可调到 `450rpm / 30Hz`，每格 `10rpm / 0.667Hz`。按 `KEY1` 后会直接爬到当前旋钮设定值，不再先冲到默认 1200rpm。OLED 的 `rpm:` 是当前开环机械转速，`set:` 是旋钮设定转速，`ol:` 是当前开环拖动电频率；底行 `r:` 是命令 q 轴电流，`q:` 是实际 q 轴反馈电流。
...
- 速度参考范围：`30-80 Hz` 电频率，对 4 对极压缩机约等价于 `450-1200 rpm` 机械转速；当前安全版不会自动上探 `120Hz+`。
- 启动电流：参考 GE2117 思路，前 `1s` 使用 `Id=3A/Iq=0A` 定位，之后使用 `Id=3A/Iq=0-2.5A` 慢拖；`Iq` 约 `6.7s` 爬到 2.5A。
- 启动角度：当前使用 `1s` 固定电角度定位，然后按 EC11 设定值做 `1-30..80Hz` 正向开环慢拖并保持，升速 `2Hz/s`、降速 `8Hz/s`；当前诊断版先不让 EKF/速度闭环接管。
- 当前验证：12V 台架下 `30/45/60 Hz` 开环保持已可稳定运行；用户反馈 `OL≈80Hz` 吸力已经满足当前产品需求。此前 `120Hz+` 上探会带来明显温升，因此当前固件保持低速开环驱动，不自动闭环接管。
...
当前分支 `feature/open-loop-pot-compressor` 已固化为低速 EC11 产品演示版：上电不自启动，按 `KEY1` 后以开环 FOC 启动并保持在旋钮设定的 `30-80Hz` 范围内。历史 EKF handoff 诊断代码仍保留在工程中，但 `EKF_HANDOFF_BLEND_ENABLE=0`，EKF 只用于显示和 trace 观察。
