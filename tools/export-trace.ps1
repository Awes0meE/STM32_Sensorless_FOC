...
if (($recordSize -ne 32) -and ($recordSize -ne 48) -and ($recordSize -ne 52) -and ($recordSize -ne 64)) {
...
if ($recordSize -ge 64) {
  $lines.Add("time_ms,ol_hz,ekf_hz,iq_ref_a,iq_fb_a,id_fb_a,ia_a,ib_a,ic_a,vbus_v,vd_v,vq_v,angle_rad,target_hz,foc_theta_rad,ekf_angle_rad,valpha_v,vbeta_v,ialpha_a,ibeta_a,ekf_angle_err_rad,ekf_speed_ratio,diag_flags,state,fault,motor,speed_flag,handoff_blend,trip_blend,trip_angle_err_rad,trip_speed_ratio,trip_reason,handoff_state,handoff_offset_rad")
} elseif ($recordSize -ge 52) {
...
for ($n = 0; $n -lt $recordsToWrite; $n++) {
  ...
  if ($recordSize -ge 64) {
    ...
    $handoffBlend = (Read-I16 $bytes ($base + 52)) / 1000.0
    $tripBlend = (Read-I16 $bytes ($base + 54)) / 1000.0
    $tripAngleErr = (Read-I16 $bytes ($base + 56)) / 1000.0
    $tripSpeedRatio = (Read-I16 $bytes ($base + 58)) / 1000.0
    $tripReason = Read-U8 $bytes ($base + 60)
    $handoffState = Read-U8 $bytes ($base + 61)
    $handoffOffset = (Read-I16 $bytes ($base + 62)) / 1000.0

    $lines.Add(("{0},{1:f1},{2:f1},{3:f2},{4:f2},{5:f2},{6:f2},{7:f2},{8:f2},{9:f2},{10:f2},{11:f2},{12:f3},{13:f1},{14:f3},{15:f3},{16:f2},{17:f2},{18:f2},{19:f2},{20:f3},{21:f3},{22},{23},{24},{25},{26},{27:f3},{28:f3},{29:f3},{30:f3},{31},{32},{33:f3}" -f `
      $timeMs,$ol,$ekf,$iqRef,$iqFb,$idFb,$ia,$ib,$ic,$vbus,$vd,$vq,$angle,$targetHz,$focTheta,$ekfAngle,$valpha,$vbeta,$ialpha,$ibeta,$ekfAngleErr,$ekfSpeedRatio,$diagFlags,$state,$fault,$motor,$speedFlag,$handoffBlend,$tripBlend,$tripAngleErr,$tripSpeedRatio,$tripReason,$handoffState,$handoffOffset))
  } elseif ($recordSize -ge 52) {
    $targetHz = (Read-I16 $bytes ($base + 28)) / 10.0
    $focTheta = (Read-I16 $bytes ($base + 30)) / 1000.0
    $ekfAngle = (Read-I16 $bytes ($base + 32)) / 1000.0
